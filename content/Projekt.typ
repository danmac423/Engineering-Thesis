...
Materiał wejściowy jest najpierw dekodowany, a uzyskana sekwencja klatek dzielona na~segmenty czasowe o zadanej długości. Długość segmentu musi spełniać zależność wynikającą z przyczynowej kompresji czasowej autoenkodera, dlatego zbyt krótki segment jest dopełniany powieleniem ostatniej klatki, a nadmiarowe klatki są odrzucane po zakończeniu przetwarzania. Mechanizm podziału czasowego omówiono w @kafelkowanie-czasowe[podrozdziale].
...
Złożony segment zapisywany jest przyrostowo, a klatki z obszaru zakładki łączone są z~segmentem poprzednim. Kluczową decyzją projektową jest przy tym przenoszenie wyniku każdego kafla poza pamięć karty graficznej bezpośrednio po jego przetworzeniu. W pamięci GPU alokowane są jednocześnie wyłącznie wagi modelu, pamięć podręczna kluczy i wartości oraz~aktywacje jednego kafla, natomiast płótna akumulacyjne obejmujące jeden segment czasowy utrzymywane są w pamięci operacyjnej.
