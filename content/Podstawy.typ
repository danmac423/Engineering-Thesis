...
Zadaniem modelu VSR jest odwrócenie tego przekształcenia, czyli znalezienie funkcji odwzorowującej $f_("VSR")$, sparametryzowanej przez wagi $theta_(f_("VSR"))$, która estymuje klatkę wysokiej rozdzielczości $hat(I)_(i)^("SR")$ na podstawie sekwencji klatek wejściowych niskiej rozdzielczości @liu2022videosuperresolutionbased:

$
  hat(I)_(i)^("SR") = f_("VSR")(I_i^("LR"), {I_j^("LR")}^(i+N)_(j=i-N)\;theta_(f_("VSR")))
$<vsr_eq>

Odwrócenie to nie ma jednoznacznego rozwiązania. Degradacja bezpowrotnie usuwa część informacji i nie jest odwracalna, ponieważ jednej sekwencji LR odpowiada wiele różnych sekwencji HR. Model nie odtwarza więc oryginału, lecz wskazuje jedno z potencjalnych rozwiązań.
...
Spójność czasowa w kontekście superrozdzielczości wideo polega na zachowaniu ciągłości i płynności oraz na eliminacji zauważalnych zniekształceń i migotania między kolejnymi klatkami wygenerowanej sekwencji HR @Baniya_2024 @liu2022videosuperresolutionbased. Bezpośrednie zastosowanie metod SISR do poszczególnych klatek pomija zależności czasowe, co prowadzi do niestabilności detali i~artefaktów wizualnych. W modelach głębokiego uczenia spójność tę realizuje się poprzez techniki wyrównywania klatek i kompensacji ruchu (ang. _motion estimation and motion compensation_, MEMC), konwolucje 3D, moduły rekurencyjne propagujące kontekst czasowy oraz mechanizmy uwagi @Baniya_2024. Choć metody te poprawiają jakość generowanych sekwencji, analiza wielu klatek znacznie zwiększa złożoność obliczeniową i pamięciową oraz stwarza ryzyko akumulacji i~przenoszenia błędów między klatkami @chan2021investigatingtradeoffsrealworldvideo.
...
Przetwarzanie w DiT rozpoczyna się od skompresowania obrazu do przestrzeni ukrytej za pomocą wstępnie wytrenowanego autoenkodera (np. autoenkodera wariancyjnego, VAE) o zamrożonych wagach. Na tak uzyskanej reprezentacji ukrytej $z_0$ przeprowadza się proces dyfuzji, a zaszumiony wektor ukryty $z_t$ dzielony jest na siatkę płatów. Następnie są one rzutowane liniowo na sekwencję tokenów i wzbogacane o sinusoidalne osadzenia pozycyjne. Kluczową innowacją architektury DiT jest sposób wstrzykiwania informacji warunkujących, takich jak krok czasowy $t$ czy etykieta klasy $c$. Do tego celu wykorzystuje się zmodyfikowane bloki z adaptacyjną normalizacją warstwową (ang. _adaptive layer normalization_, adaLN-Zero). Zamiast standardowej normalizacji, sieć estymuje parametry skali i przesunięcia $(gamma, beta)$ oraz skalar przeskalowujący $alpha$ bezpośrednio z sumy wektorów osadzeń warunków. Po przejściu przez bloki DiT, wyjściowa sekwencja tokenów jest dekodowana liniowo i reorganizowana do pierwotnego kształtu reprezentacji ukrytej, skąd dekoder przekształca ją z powrotem do~przestrzeni pikseli @peebles2023scalablediffusionmodelstransformers.

Głównym wyzwaniem w adaptacji architektur DiT do zadań superrozdzielczości wideo jest potrzeba uwarunkowania procesu generatywnego nagraniem LR. W modelach VSR opartych na DiT (takich jak _FlashVSR_, omawiany w @model-flashvsr[podrozdziale]) sekwencja HR nie jest generowana z czystego szumu; zamiast tego do sterowania odtwarzaniem detali wykorzystuje się wideo LR. Wprowadzanie informacji warunkującej realizuje się m.in. poprzez rzutowanie klatek LR za pomocą lekkiej warstwy konwolucyjnej (_Proj-In_) i dodanie tak uzyskanych cech do tokenów jako osadzenia warunkujące @Zhuang2025FlashVSRTR.

== Optymalizacje mechanizmu uwagi
<optymalizacje-mechanizmu-uwagi>

Standardowy mechanizm uwagi oparty na iloczynie skalarnym ze skalowaniem (ang. _Scaled Dot-Product Attention_) stanowi fundament architektury Transformer. Odwzorowuje on zapytania ($Q$), klucze ($K$) oraz wartości ($V$) na wektor wyjściowy zgodnie z zależnością @vaswani2023attentionneed:
...
W celu przeprowadzenia kompleksowej ewaluacji jakości wygenerowanych sekwencji wideo zastosowano zestaw siedmiu metryk badawczych, obejmujący zarówno tradycyjne wskaźniki pełnoreferencyjne (ang. _Full-Reference_), jak i zaawansowane metryki bezreferencyjne (ang.~_No-Reference_).
