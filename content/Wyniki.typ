...
Efekty tych osi sumują się. Najniższy szczyt w całym zestawieniu, 8112 MiB, osiąga konfiguracja łącząca SpargeAttention z kwantyzacją wag i aktywacji. Jest to o 11,0% mniej niż w konfiguracji odniesienia, przy czasie dłuższym o 15,7%. Na uwagę zasługuje jednak inna konfiguracja. SpargeAttention z kwantyzacją samych wag daje szczyt 8766 MiB i czas 24,2 s. Jest więc jednocześnie oszczędniejsza pamięciowo i o 9,9% szybsza od konfiguracji wyjściowej. To jedyny badany wariant poprawiający obie mierzone wielkości naraz.
...
W konfiguracji odniesienia kafel 224 kończy się przepełnieniem pamięci. Ten sam kafel po zastosowaniu SpargeAttention oraz kwantyzacji wag i aktywacji przetwarzany jest ze szczytem 8920 MiB, a więc z wyraźnym zapasem względem konfiguracji, przy której nastąpiło przepełnienie.
...
Model w postaci udostępnionej przez autorów nie uruchamia się na karcie docelowej przy żadnej z badanych rozdzielczości wejściowych. Warunkiem koniecznym uruchomienia jest kafelkowanie przestrzenne, ponieważ tylko ono uniezależnia szczytowe zużycie pamięci od rozdzielczości nagrania. Pozostałe badane techniki zmieniają je o nie więcej niż 11%, co wobec skali deficytu jest niewystarczające.

Rozmiar kafla oddziałuje na zużycie pamięci wyraźnie silniej niż którakolwiek z technik obliczeniowych. Zmniejszenie kafla ze 192 do 128 obniża szczyt o 2820 MiB, podczas gdy kwantyzacja wag i aktywacji obniża go o 898 MiB. Kafelkowanie jest przy tym jedyną techniką o zauważalnym koszcie jakościowym, pozostałe są pod tym względem w przybliżeniu neutralne.

Dobór konfiguracji przedstawia się zatem następująco. Kafelkowanie przestrzenne należy włączyć zawsze, a rozmiar kafla dobrać do rozdzielczości wejściowej. Dla rozdzielczości badanej najkorzystniejszy okazał się kafel 160. Selekcję rzadką warto zastąpić SpargeAttention, ponieważ skraca czas o blisko 13% przy niewielkim koszcie jakościowym. Jądro uwagi gęstej należy pozostawić bez zmian, gdyż jego podmiana nie przynosi korzyści w żadnym wymiarze. Kwantyzację wag warto włączyć zawsze, gdyż daje konfigurację jednocześnie szybszą i oszczędniejszą od wyjściowej, natomiast kwantyzację aktywacji jedynie wtedy, gdy zapas pamięci okazuje się niewystarczający.
